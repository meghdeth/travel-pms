...
export interface BedBookingGuest {
  first_name: string;
  last_name: string;
  email: string;
  phone: string;
...
export interface BedBookingPayment {
...
export class BedBooking extends BaseModel {
  ...
  // Pricing
  pricing!: BedBookingPricing;

  // Payment
  payment!: BedBookingPayment;
  ...
  static get jsonSchema() {
    return {
      type: 'object',
      required: ['booking_reference', 'hotel_id', 'room_id', 'bed_id', 'guest_details', 'check_in_date', 'check_out_date', 'nights', 'pricing', 'payment', 'status', 'booking_source'],
      properties: {
        ...super.jsonSchema.properties,
        booking_reference: { type: 'string', minLength: 1, maxLength: 50 },
        ...
        pricing: {
          type: 'object',
          required: ['bed_rate', 'tax_amount', 'service_charge', 'discount_amount', 'total_amount', 'currency'],
          properties: {
            bed_rate: { type: 'number', minimum: 0 },
            tax_amount: { type: 'number', minimum: 0 },
            service_charge: { type: 'number', minimum: 0 },
            discount_amount: { type: 'number', minimum: 0 },
            total_amount: { type: 'number', minimum: 0 },
            currency: { type: 'string', minLength: 3, maxLength: 3 },
            meal_plan: { type: ['string', 'null'], enum: ['none', 'breakfast', 'half_board', 'full_board'] },
            meal_cost: { type: ['number', 'null'], minimum: 0 }
          }
        },
        payment: {
          type: 'object',
          ...
          properties: {
            payment_method: { type: 'string', enum: ['credit_card', 'debit_card', 'paypal', 'stripe', 'bank_transfer', 'cash'] },
        ...
        booking_source: { type: 'string', enum: ['direct', 'ota', 'phone', 'walk_in', 'agent'] },
        ...
        locker_number: { type: ['string', 'null'], maxLength: 20 },
        key_card_number: { type: ['string', 'null'], maxLength: 50 },
        emergency_contact: { type: ['string', 'null'], maxLength: 100 },
        emergency_phone: { type: ['string', 'null'], maxLength: 20 }
      }
    };
  }
  ...
  //   return `BED-${timestamp}-${random}`.toUpperCase();
  // }
  ...
  canCheckIn(): boolean {
    return this.status === 'confirmed' && new Date() >= new Date(this.check_in_date);
  }
